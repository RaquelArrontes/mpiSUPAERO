...
{-1,-1,-1}};
  ...
  for (i = 1; i < h ; i++)
    for (j = 1; j < w ; j++) {
         image_out[i][j] = kernel[0][0]*image_in[i-1][j-1]+kernel[0][1]*image_in[i-1][j]+kernel[0][2]*image_in[i-1][j+1]+kernel[1][0]*image_in[i][j-1]+kernel[1][1]*image_in[i][j]+kernel[1][2]*image_in[i][j+1]+kernel[2][0]*image_in[i+1][j-1]+kernel[2][1]*image_in[i+1][j]+kernel[2][2]*image_in[i+1][j+1];
    }
